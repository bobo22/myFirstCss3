...
define(function(require, exports, module) {
	...
	function setPullDownFunc() {
		...
		var scrollFunc = function() {
			var y = WindowTools.dimensions.getScrollTop();
			var slider = document.getElementById('sliderSegmentedControl');

			if((y + WindowTools.dimensions.getWinHeight()) === WindowTools.dimensions.getScrollHeight()) {
				if(!instance.loadingUp) {
					if(!instance.finished) {
						if(instance.options.up) {
							//加载更多
	...
	function loadMoreAnimation(more) {
		...
		if(more) {
			if(!instance.loadingUp) {
				...
				WindowTools.dom.appendHtmlChildCustom(dom, pulluploadingTips);
