	...
	(function() {
		...
		function dealUrl(url, isCommondto) {
			...
			if(url.indexOf('.') != -1 && url.indexOf('.jspx') == -1) {
				if(window.epointmConfig&&epointmConfig.isRestFul){
					url = url.replace('.', '/');
					...
					success: function(data) {
						...
						if(code >= 300) {
							if(url) {
								...
								url = epm.getRightUrl(url);
								mui.alert('错误:'+JSON.stringify(status));
								return ;
								if(status.top) {
		...
		var epointm = {
			...
			alert: function(message, title, callback) {
				mui.alert(message,title,'我知道了',callback);
				
			},

			confirm: function(message, title, okCallback, cancelCallback) {
				mui.confirm(message,title,['确定', '取消'],function(index) {
					// 确定
